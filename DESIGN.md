...
로컬 LLM 데스크톱 노트 앱을 위한 디자인 언어. 목표는 **글에 집중하게 만드는 조용한 도구**다.
UI는 종이(canvas)와 잉크(ink)의 무채색으로 물러나고, 색은 *의미를 가질 때만* 나타난다.
세 가지 신호색만 존재한다 — **보라 = AI의 행동**, **teal = 로컬/안전·1차 기능**, **amber = 2차(예정)**.

핵심 정서: 신뢰(데이터가 기기 밖으로 안 나감) · 집중(글쓰기 우선) · 협업(AI는 조용한 동료).
...
- 단계 색과 의미 색을 통일: 1차/성공/로컬은 teal, 2차/경고/대기는 amber. 사용자가 색만 봐도 상태를 읽는다.
...
### Do
- 색은 의미가 있을 때만. 보라는 AI, teal은 로컬/1차, amber는 2차.
...
- 온보딩/설정/빈 상태(empty state) 화면 토큰은 추후 추가.
