...
## 2026-07-15
- **챗 입력 `+` 도구 선택기 (Claude식)**: 입력창 왼쪽 **`+`** → 사용 가능한 도구를 **이름+설명**과 함께 표시(발견성), 선택하면 보라 칩으로 뜨고 **그 도구만 강제 사용**(`tool_choice`: 1개면 해당 function 강제, 여러 개면 required). 미선택 시 기존대로 auto. `NimClient.chatTools`/`ChatEngine.send`에 `toolChoice` 추가.
- **챗 UX 확장 — AI 토글 오버레이 + 도구 관리 + 모델 전환**: (1) 기본은 노트 전체 폭, **우상단 `✦ AI` 토글**로 AI 챗을 우측 오버레이로 열고 닫음(편집기 안 가림). (2) 설정에 **도구 on/off**(커넥터식) — `ToolCatalog` 목록을 토글, 켠 도구만 AI가 사용(`SecureStore.isToolEnabled`, 요청마다 registry 구성). (3) AI 헤더에서 **모델 즉시 전환**(드롭다운, 저장). 모델 목록은 `SecureStore.MODELS`로 공용화. 두 상태 실행 검증.
